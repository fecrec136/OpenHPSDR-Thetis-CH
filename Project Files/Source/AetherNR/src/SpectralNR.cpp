...
#include "aethernr_support.h"   // Thetis: zlib/base64/SHA-256/logging in place of Qt
#include <cassert>
#include <algorithm>
#include <bit>
#include <cstdio>
...
const GammaGainTables& gammaGainTables()
{
    static const GammaGainTables tables = [] {
        ...
        // Qt's qUncompress(): 4-byte big-endian length prefix + zlib stream
        const std::vector<unsigned char> shuffled =
            aethernr::qUncompress(aethernr::fromBase64(encoded));
        const std::size_t expectedBytes =
            static_cast<std::size_t>(kGammaTableValueCount)
            * sizeof(std::uint64_t);
        if (shuffled.size() != expectedBytes) {
            return result;
        }

        const auto* packed = shuffled.data();
        ...
        for (int index = 0; index < kGammaTableValueCount; ++index) {
            ...
            for (int byte = 0; byte < 8; ++byte) {
                rawBytes[8 * index + byte] = static_cast<unsigned char>(
                    bits >> (8 * byte));
            }
        }

        const std::string actualSha256 = aethernr::sha256Hex(rawBytes);
        result.valid = actualSha256 == kNr2GammaTablesRawSha256;
        return result;
    }();
    return tables;
}
...
bool exportWisdomAtomically(const std::string& directory)
{
    ...
    if (!fftw_export_wisdom_to_filename(tempFile.c_str())) {
        std::remove(tempFile.c_str());
        aethernr::logWarning("SpectralNR: failed to export FFTW wisdom to " + tempFile);
        return false;
    }

    std::remove(wisdomFile.c_str());
    if (std::rename(tempFile.c_str(), wisdomFile.c_str()) != 0) {
        aethernr::logWarning("SpectralNR: failed to install FFTW wisdom at " + wisdomFile);
        std::remove(tempFile.c_str());
        return false;
    }
    ...
    , m_useLegacyGainMethods(useLegacyGainMethods)
{
    ...
    if (!m_planFwd || !m_planRev) {
        aethernr::logWarning("SpectralNR: FFTW plan creation failed - NR2 will not function");
        m_planFailed = true;
    }
#else
                                                    ...
                                                    WisdomCancelCb shouldCancel)
{
    ...
    if (!cbuf || !rbuf) {
        freeBuffers();
        std::remove(wisdomTempPathForDirectory(directory).c_str());
        aethernr::logWarning("SpectralNR: failed to allocate buffers for FFTW wisdom");
        return WisdomResult::Failed;
    }
...
void SpectralNR::process(const float* input, float* output, int numSamples)
{
    ...
    assert(m_outputAvailable >= numSamples);
    for (int i = 0; i < numSamples; ++i) {
...
void SpectralNR::processStereoSharedMask(const float* input, float* output, int numFrames)
{
    ...
    assert(m_outputAvailable >= numFrames);
    for (int i = 0; i < numFrames; ++i) {
